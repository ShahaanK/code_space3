...
**Status update (2026-07-30):** **Production wave LIVE** — Llama 3.3 70B (676996), Qwen 2.5 72B (676997), and Mistral Small 3.1 24B (677046) at 57 chunks each, run0001 per model, 171 jobs. Run numbering is now per-model (M8.T32). Four latent production bugs found and fixed mid-flight (M8.T33), incl. OG's 1024 fd soft limit. Falcon kernel-blocked (M8.T34), AceGPT download stalled (M8.T37), DeepSeek pending M8.T29. A tracked Slack webhook was purged from history and a pre-push secret scanner added (M8.T36) — **the exposed webhook still needs rotating (M8.T38)**.
...
- [x] M8.T10i — F1 regression investigation — **CLOSED 2026-07-30: no regression exists.** The 0.208 was a small-sample artifact (n=100, only 22/25 labels had gold positives). Evaluating the existing OG chunk_000 files at **n=1000** (all 25 labels populated) gives Llama 3.3 incentive_strict macro F1 = **0.267**. March's 0.285 is not a valid comparator — different model (3.1), different config (config2), and a disjoint 50-text draw. No longer gates the production wave
...
- [⏳] M8.T12 — Submit full 56K production runs (subset protocol per Introne: first chunk per model, verify, then release the rest); Llama + Qwen first, DeepSeek gated on M8.T10h. Launched 2026-07-30 via `submit_wave.sh` with per-model run counters: Llama 3.3 70B (57 chunks, cluster 676996, run0001), Qwen 2.5 72B (57 chunks, 676997, run0001), and Mistral Small 3.1 24B (57 chunks, 677046, run0001) after its gate passed — **171 jobs in production**. Subset protocol waived: Llama and Qwen already had complete, clean 5-arm chunk_000 results on OG (0.06% -1 each), so no separate verification chunk was needed. DeepSeek held pending M8.T29; AceGPT and Falcon pending M8.T27.
...
- [⏳] M8.T27 — Validation gate for the 3 unvalidated new models. **Mistral ✅ PASSED 2026-07-30** on the actual roster model (0.00% -1, exit 0, V1 engine, ready in 140s) → full wave launched. **Falcon 🚫 kernel-blocked** (see M8.T34). **AceGPT ⏳** — download stalled at 132G of ~140G; sub reconfigured for fp16 TP=2 (see M8.T37)
...
- [⏳] M8.T29 — Re-verify DeepSeek approved stochastic config in production: actual gate-test result (job 667153, completed 2026-07-23) shows a **44.18% -1 rate**, contradicting the "resolved" framing in M8.T10h/T26 (worse than the 36.5% that got `repetition_penalty=1.15` rejected). Two isolated diagnostic reruns launched on OG to test root cause — no-system-prompt (job 677000) and `<think>` assistant-prefill (job 677001) — both in progress as of 2026-07-30
- [x] M8.T30 — Rebuilt condor_alert.sh from scratch: fixed a revoked Slack webhook + `WEBHOOK_URl`/`WEBHOOK_URL` case typo, converted from a one-shot single-log-file watcher to a standing `condor_q`-based watcher covering all jobs automatically (HELD/auto-released/FAILED alerts + 6h digest). Deployed and Slack-verified on OG
- [x] M8.T31 — Built neg1_watch.sh (new, Apophis-side): polls OG for completed jobs, pulls results back, computes -1 rate via pandas locally, Slack-alerts above a 10% threshold — keeps result analysis on Apophis per project convention. Deployed and Slack-verified
- [x] M8.T32 — Per-model run counters: `next_run.sh` now takes a model tag and increments `run_counter_<tag>.txt`, so every model's first wave is run0001 and a re-submit of that model is run0002 (RUN_NO stays the attempt number, not the chunk index — required for select_and_merge.py's latest-complete-run selection). Added `chunklist_all.txt` (000–056); `submit_wave.sh` pre-validates the chunklist against `$PWD`. Verified counter independence, seed handling, and 8-way flock concurrency (2026-07-30)
- [x] M8.T33 — Four mid-flight production fixes: (a) per-job vLLM logs at `results_<tag>/<chunk_stem>.vllm.log` — all jobs previously opened the same `hpc/vllm_server.log` with mode `"w"` and interleaved, destroying diagnostics; (b) vLLM startup timeout 600s→2400s (`CAMEL_VLLM_TIMEOUT`), too tight under a 100+ job NFS stampede; (c) `wrapper.sh` raises the fd soft limit to the hard limit — **OG defaults to 1024 vs Apophis's 1,048,576**, causing `[Errno 24] Too many open files`; (d) `start_vllm` omits `--quantization` for `none`/`auto` so unquantized checkpoints can load (2026-07-30)
- [x] M8.T34 — Falcon-H1-34B diagnosed: GPTQ-Int4 cannot use marlin (`output_size_per_partition=9248` not divisible by `min_thread_n=64`; Machete needs sm_90 vs our sm_86, BitBLAS/Conch absent, Exllama fp16-only) → switched to plain `gptq`. Also confirmed it initializes a **V0 engine**, the SSM/hybrid limitation, so expect ~3× slower than V1 even once loading succeeds. Procedural lesson: releasing a held job reuses submit-time ClassAds, so `.sub` edits do not apply — fixed via `condor_qedit` on `Environment` (not the legacy `Env`) (2026-07-30)
- [x] M8.T35 — Mistral model-identity correction: the 07-21 gate had validated `stelterlab/Mistral-Small-24B-Instruct-2501-AWQ` (Mistral Small 3), not the roster model `RedHatAI/Mistral-Small-3.1-24B-Instruct-2503-quantized.w4a16` (Mistral Small 3.1) that the production sub actually points at. Downloaded and re-gated the roster model; AceGPT repo id also corrected to case-sensitive `-Chat` (2026-07-30)
- [x] M8.T36 — Slack webhook secrets incident: `hpc/condor_alert.sh` was tracked with a live webhook across 20 published commits. Untracked it, purged from all 55 commits via `filter-branch`, force-pushed, verified 0 commits retain the file or URL; deleted `condor_alert_old.sh` (last on-disk copy of the exposed value). Externalized the current webhook to `~/.camel_secrets.env` (chmod 600, outside the repo), sourced by both alert scripts, mirrored to OG. `.gitignore` now pattern-based (`hpc/*alert*.sh`, `hpc/*watch*.sh`). Added tracked `.githooks/pre-push` secret scanner with redacted output. Independent audit confirmed no other credentials in tree, history, or PR refs (2026-07-30)
- [ ] M8.T37 — AceGPT-v2-70B production config: no AWQ/GPTQ/INT4 release exists (GGUF only), so it runs unquantized fp16 at TP=2 across both A100s on a node (~140GB weights against 160GB, leaving thin KV headroom). Fallback if batching is starved: on-the-fly fp8 on 2× L40S (sm_89 native FP8; A100 sm_80 would emulate). **Download stalled at 132G of ~140G — resume before gating.** Note the fp16-vs-INT4 quantization confound this introduces into the East/West comparison and RQ4
- [ ] M8.T38 — **ROTATE the exposed Slack webhook** (`3ca7370b`, the one from `condor_alert_old.sh` — NOT the current `c9bcfd62`, which was never committed). GitHub still serves the purged commit and blob by SHA until GC, so the rewrite alone does not revoke it. Optionally request a GitHub Support GC for commit `efee473` / blob `219fbe5`
...
### Side Items
- [ ] Slack → Claude Code terminal listener (someday, low priority) — reverse direction of condor_alert.sh/neg1_watch.sh: a listener process running on Apophis itself that receives a Slack message and spawns a local Claude Code CLI session with full local access (SSH to OG, bash, filesystem). Confirmed 2026-07-30: neither Claude Tag (Claude in Slack) nor cloud `/schedule` routines can do this — both run in an Anthropic-hosted sandbox that explicitly blocks SSH/non-HTTP access to local machines, so they can't reach Apophis or OG. A local listener (Slack Events API or polling, run on Apophis) is the only path that preserves local tool access. Needs an inbound-reachable endpoint (ngrok/Cloudflare Tunnel or polling) plus guardrails on what an incoming message can trigger.
...
### 2026-07-30
- (Shahaan) ✅ M8.T10i — F1 regression CLOSED: no regression exists. 0.267 at n=1000 vs the 0.208 small-sample artifact at n=100; March's 0.285 was never a valid comparator (different model, config, and disjoint draw). Unblocks the wave
- (Shahaan) ⏳ M8.T12 — 56K production wave LAUNCHED: Llama (676996) + Qwen (676997) + Mistral 3.1 (677046), 57 chunks each at run0001 = 171 jobs
- (Shahaan) ✅ M8.T32 — Per-model run counters (`run_counter_<tag>.txt`) + `chunklist_all.txt`; flock concurrency verified
- (Shahaan) ✅ M8.T33 — Four mid-flight production fixes: per-job vLLM logs, 2400s startup timeout, fd soft limit 1024→1048576 on OG, unquantized `--quantization` bypass
- (Shahaan) ✅ M8.T34 — Falcon-H1 diagnosed: marlin-incompatible (9248 not divisible by 64) → plain `gptq`; also V0-engine-bound. `condor_qedit` needed because release reuses submit-time ClassAds
- (Shahaan) ✅ M8.T35 — Mistral roster-model identity corrected (3.1/RedHatAI, not 3/stelterlab); AceGPT repo capitalization fixed
- (Shahaan) ✅ M8.T36 — Webhook secrets incident remediated: history purged, force-pushed, secret externalized to `~/.camel_secrets.env`, pattern-based gitignore, tracked pre-push scanner added; independent audit found no other credentials
- (Shahaan) ⏳ M8.T27 — Mistral gate ✅ passed (0.00% -1); Falcon 🚫 kernel-blocked; AceGPT ⏳ download stalled
- (Shahaan) 🆕 M8.T37 — AceGPT fp16 TP=2 config + stalled download; flags an fp16-vs-INT4 confound for RQ4
- (Shahaan) 🆕 M8.T38 — ROTATE the exposed webhook; GitHub still serves the purged objects by SHA
- (Shahaan) 🆕 M8.T29 — Re-verified DeepSeek's approved stochastic config against the actual job 667153 gate-test data: 44.18% -1 rate, contradicting the "resolved" framing in M8.T10h/T26. Two isolated diagnostic reruns launched (no-system-prompt job 677000, `<think>`-prefill job 677001), ⏳ in progress
- (Shahaan) ✅ M8.T30 — condor_alert.sh rebuilt: fixed dead webhook + case typo, converted to a standing condor_q watcher (HELD/auto-released/FAILED + 6h digest), covers all jobs automatically
- (Shahaan) ✅ M8.T31 — neg1_watch.sh built (Apophis-side -1 rate sentinel, 10% alert threshold, keeps analysis off OG)
- (Shahaan) ⏳ M8.T12 — Confirmed underway: full Llama+Qwen 56K production wave (114 chunks) plus Mistral/Falcon gate tests observed running on OG (launched outside this session)
- (Shahaan) 🆕 Side Items — Logged Slack→Claude-Code-terminal listener as a low-priority backlog idea; confirmed not currently feasible via Claude Tag or cloud `/schedule` routines (sandbox blocks local/SSH access)
- (Shahaan) 🔧 Also found and fixed: `/deepseek-triage` workflow was silently analyzing a hardcoded stale file instead of the args passed in — corrected manually, filed as a workflow bug
