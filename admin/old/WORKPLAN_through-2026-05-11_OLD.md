...
### Milestone 8: OrangeGrid HPC Setup & Production Runs ⚡ NEW (post-4/30)
**Status:** Account approved 4/29 by Dan/ITS. Login: its-og-login5.syr.edu. Connection via SU RDS. Production 56K runs target this cluster, not Apophis.

- [x] M8.T1 — Request OrangeGrid access (approved 4/29)
- [ ] M8.T2 — First SSH connection to its-og-login5.syr.edu via SU RDS
- [ ] M8.T3 — Empirical reconnaissance (login-node diagnostic commands): GPU pool inventory, ClassAd attribute discovery, default request_disk, runtime/preemption rules, Singularity availability
- [ ] M8.T4 — Email researchcomputing@syr.edu with remaining ITS questions (cc Introne) — only after empirical recon leaves gaps
- [ ] M8.T5 — Set up OrangeGrid environment (conda or venv per ITS conventions; light setup on login node only)
- [ ] M8.T6 — Pull vllm-openai.sif Singularity container
- [ ] M8.T7 — Pre-cache HuggingFace model weights to /home/szkhan (Llama 3.3 70B, Qwen 2.5 72B, DeepSeek-R1 32B AWQ)
- [ ] M8.T8 — Clone code from GitHub to OrangeGrid; rsync chunks/ from Apophis
- [ ] M8.T9 — Adapt batch scripts for OrangeGrid: TP=1, request_gpus=1, +request_gpus=1 ClassAd, request_disk back in, target A100 80GB via VRAM threshold (verified attribute name from M8.T3)
- [ ] M8.T10 — Submit OrangeGrid 100-text test job (Llama 3.3 70B); validate Feather output
- [ ] M8.T11 — Submit full 56K production runs across all 3 validated models in parallel (12 chunks × 3 models)
- [ ] M8.T12 — Monitor with condor_q -better-analyze if jobs idle; hot-fix held jobs in place; condor_release to resume
...
### Side Items
- [ ] DGX Spark deliverable (in progress) — local AI deployment comparison notes promised to Prof. Introne for the businessman conversation. Full landscape report drafted 4/30 covering DGX Spark, Mac Studio, GPU workstations, cloud. Pending share to Prof. Introne.
...
## Reference: Cluster Topology (4/30)

| | Apophis | OrangeGrid |
|---|---------|------------|
| Owner | Prof. Introne (iSchool lab) | Syracuse ITS |
| GPUs | 2× RTX A6000 (48 GB, sm_86) | Hundreds: A100 80GB / L40S 48GB / A6000 / A40 (via SUrge) |
| Total VRAM | 96 GB on one box | Per-job, anywhere in pool |
| Scheduler | None (direct vLLM) | HTCondor |
...
| Status | Working, validated | Approved 4/29, setup pending |
