...
def split_data(
    ...
    if create_test:
        ...
        # Generate batch submit file
        generate_batch_submit(output_dir, chunk_manifest)


def generate_batch_submit(output_dir: Path, chunk_manifest: list):
    """Generate HTCondor batch submit file for all chunks"""
    submit_path = output_dir.parent / 'batch_job.sub'

    # Get the parent directory (where batch_job.sub will live)
    script_dir = output_dir.parent.resolve()

    # Derive subdirectory names from the actual output dir
    chunks_subdir = output_dir.name  # e.g. "chunks" or "my_chunks"
    ...
    for chunk in chunk_manifest:
        ...
        result_file = f"{results_subdir}/results_{chunk['chunk_id']:03d}.feather"
        queue_items.append(f"    {chunk_file}, {result_file}, chunk_{chunk['chunk_id']:03d}")

    queue_list = "\n".join(queue_items)

    submit_content = f"""#!/usr/bin/env condor_submit
# =============================================================================
# HPC Belief Extraction - BATCH JOB
# =============================================================================
# Auto-generated batch submission for {len(chunk_manifest)} chunks
...
# Monitor:
#   condor_q $USER
#   watch -n 30 'ls -la results/*.feather | wc -l'
# =============================================================================

# !! IMPORTANT: All paths are relative to this directory !!
...
# Target L40s with CUDA 12.8+ and sufficient VRAM
...
"""
    ...
    print(f"Generated batch submit file: {submit_path}")
